...
5. 設定画面の「接続テスト」が成功する
6. 記事URLからプレビューを作成できる
7. WordPressへ下書き保存できる
8. WordPress管理画面で記事が「下書き」になっている
...
- 設定画面の「接続テスト」を実行し、表示されたエラーを確認する
- 「推定原因」「判断根拠」「通信結果」に表示されたHTTPステータス、応答形式、
  最終URL、WordPressエラーコードを確認する
...
接続テストでは、代表的な結果を次のように診断します。

| 結果 | 主な推定原因 |
| --- | --- |
| HTTP 401 | 認証情報の不一致、環境変数未反映、Authorizationヘッダーの削除 |
| HTTP 403 + HTML | WAF、CDN、国外アクセス制限、RenderのIP拒否 |
| HTTP 403 + JSON | WordPressまたはセキュリティプラグインによる権限拒否 |
| HTTP 404 | WordPress URLの間違い、REST API無効化 |
| ログイン画面へ転送 | Basic認証やログイン強制機能によるREST API遮断 |
| JSON以外の応答 | WAF、メンテナンス画面、CDN、サーバー認証の割り込み |
| SSLエラー | 証明書の期限切れ、ホスト名不一致、証明書チェーンの問題 |
| タイムアウト | サーバー障害、国外アクセス制限、ファイアウォール |
